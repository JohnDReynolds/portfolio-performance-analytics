...
class TestOperationalDemoDataGenerator(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.security_master = pd.read_csv(
            _SECURITY_MASTER_PATH,
            header=None,
    ...
    def test_write_outputs_creates_reviewable_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_directory:
            ...
            written_holdings = pd.read_csv(paths["axys_a_holdings"])
            self.assertIn("Security Symbol", written_holdings.columns)
            self.assertIn("Security Type", written_holdings.columns)
            self.assertNotIn("SEC", written_holdings.columns)
