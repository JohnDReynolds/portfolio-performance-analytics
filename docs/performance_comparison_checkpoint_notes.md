...
- Generated bundle `README.md` files now use role-based sections:
  `Primary Review Artifact`, `Recommended Review Order`, and `Audit/Export
  Files`. The review order calls out transaction and external-flow diagnostic
  CSVs as audit aids whose cross-check rows may be evidence-only or
