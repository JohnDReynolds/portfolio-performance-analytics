...
[`README.md`](../../scripts/generic_analytics_demo_data/README.md).
The active safety-net program and audited enforcement baseline are defined in
the [safety invariant catalog](../audit/safety_invariants.md).

[axys-apx-blockers]: ../axys_apx/reference/Chapter_01_Overview.md#axys_apx-blockers
...
sequence live in
[`safety_invariants.md`](../audit/safety_invariants.md).
...
| Axys/APX Python package boundary | Consider renaming the importable `ppar.axys_apx` package to `ppar.axys_apx` so the public Python API matches the Axys/APX product positioning. Treat this as a deliberate API rename, not a mechanical docs cleanup. | Imports, pyproject package metadata, tests, docs, and examples all use the chosen package boundary. Decide explicitly whether `ppar.axys_apx` remains as a short-lived compatibility shim or is removed before first public PyPI release. Class names may remain `Axys...` initially if that keeps the change focused. Keep this separate from the future `vendor: axys` YAML keyword, which can remain unchanged until preset semantics are implemented. |
...
`scripts/operational_demo_data/audit_transaction_scenarios.csv`.
...
2. validated explicit residual holding scenarios in
   `scripts/operational_demo_data/audit_holding_scenarios.csv`.
...
- Axys/APX IMEX transaction codes `li`, `lo`, `dp`, and `wd` are not safe
  ...
  `ppar/setup_templates/axys_apx_performance_comparison/demo_extract_availability.yaml`, the transaction loader
...
- `docs/axys_apx/contracts/transaction_semantics_matrix.yaml` is the
  ...
  review-only/unknown.
- [Transaction semantics matrix](../axys_apx/contracts/transaction_semantics_matrix.md)
...
surface: demo resources under `ppar/setup_templates/axys_apx_performance_comparison` stay limited to CSV, YAML,
...
- `audit_transaction_scenarios.csv` derives Snapshot B
  `transactions.csv` from Snapshot A plus explicit transaction scenarios.
- `audit_holding_scenarios.csv` supplies explicit holding
  adjustments that are not safely inferable from simple transaction deltas.
- `rebuild_audit_demo_data.py` derives `holdings.csv`,
...
`axys_apx_performance_comparison.yaml`. Test-only validation and site-variant
...
`axys_apx_performance_comparison.yaml` failed as expected because the YAML uses
