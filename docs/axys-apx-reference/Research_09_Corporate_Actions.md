# Research Notes: 09-Corporate-Actions.md

**Repository area:** `research/`  
**Target chapter:** `docs/09-Corporate-Actions.md`  
...
|---|---|---|---:|---|
| Corporate actions are a distinct operational domain for Advent users; SS&C offers Advent Corporate Actions (ACA) as a separate corporate actions solution. | Unknown integration details | ACA is marketed specifically “for APX” | Verified | SS&C product brief describes ACA for APX and its APX integration. |
...
| APX provides a Reorg Utility used in the ACA workflow. | Not applicable from public source | Yes | Verified | SS&C ACA/APX brief says the Reorg Utility runs and transactions post to APX Trade Blotter. |
...
| Merger | Security reorganization; close old/open new security; possible cash-in-lieu | Unknown | ACA/APX workflow handles “reorg activities” generally | Verified for general ACA reorg support; Unknown for exact postings | Need APX Reorg Utility documentation. |
| Spin-off | Security reorganization; new security; basis allocation | Unknown | ACA/APX workflow likely supports complex events via review, but exact posting Unknown | Medium Confidence | ACA specialists help interpret complex events; specific APX transaction outputs Unknown. |
...
| Symbol/ticker/name change | Security master update; possible cross-reference update | Unknown | Unknown | Unknown | Need security master chapter/material. |
...
| Manual entry vs automated feed | Unknown from supplied/public evidence. | Unknown | ACA evidence is APX-specific in the vendor brief. |
...
| ACA reports | Not Axys from public source | APX/ACA has “reports” per product positioning | Verified for existence of ACA reporting capability only | Exact report names/fields Unknown. |
...
| Transaction | Cash dividends, reinvestments, reorg postings, cash-in-lieu | `.cli` files used for transactions in conversion | APX Trade Blotter and posted transactions likely relevant; exact storage Unknown | High Confidence for Axys conversion; Verified for APX Trade Blotter in ACA workflow |
| Holding | Used to determine action eligibility | Derived from transactions/positions in Axys; exact source Unknown | APX sends holdings to ACA Server | Verified for APX ACA workflow; Unknown for Axys |
| Corporate action instruction | Vendor/ACA event record and processing instruction | Unknown | ACA creates action records and transaction instructions, but exact APX data model Unknown | Verified at workflow level only |
...
3. APX has a vendor-supported Advent Corporate Actions workflow through ACA.
4. In the ACA/APX workflow, APX holdings can be sent to ACA, ACA creates action records, users review/download actions, the APX Reorg Utility runs, and resulting transactions post to the APX Trade Blotter.
5. Exact corporate action IMEX objects, REP reports, APX tables, Axys split field layouts, and transaction codes remain Unknown unless additional source material is supplied.
