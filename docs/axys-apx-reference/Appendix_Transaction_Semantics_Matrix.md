...
| `;` | Journal, Other, Split marker in integration tables. | `corporate_action` or `unknown pending review` | Usually none or context-dependent. | `neutral`, `review_only`, or `unknown` | Corporate-action/report context, split details, local mapping. | Low | Medium | Covered in packaged demo as review/corporate-action evidence. | Treat as integration behavior, not official native code. |
