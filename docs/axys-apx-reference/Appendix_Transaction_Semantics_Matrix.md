...
| `ai` | Negative interest or margin interest. | `fee_expense` or `income` pending local mapping. | Usually negative for margin/negative interest; context-dependent. | `performance` | Security type, margin symbols, amount sign, local mapping or REP/report semantics. | Medium | Medium | Not yet covered in packaged performance demo. | Packaged demo uses `holdings.accrued` plus `in` interest; do not infer `ai` from code alone or describe it as generic accrued interest. |
| `sa` | Sale accrued interest / sell-side accrued interest. | `income` or `fee_expense` pending local mapping. | Context-dependent. | `performance` or `unknown` | Bond/accrual context, accrued-interest amount, settlement date, local mapping or REP/report semantics. | Medium | Medium | Not yet covered. | Requires sell-accrued-interest fixture before packaged-demo use. |
| `pa` | Purchase accrued interest / buy-side accrued interest. | `fee_expense` or `income` pending local mapping. | Context-dependent. | `performance` or `unknown` | Bond/accrual context, accrued-interest amount, settlement date, local mapping or REP/report semantics. | Medium | Medium | Not yet covered. | Requires purchase-accrued-interest fixture before packaged-demo use. |
...
| `pd` | Principal paydown / bond-security return-of-capital transaction. | `income` or `corporate_action` pending local policy. | Usually positive cash with reduced principal exposure. | `performance` or `review_only` depending on configured methodology. | Bond/security type, principal/paydown context, cash movement, factor or principal reduction. | Medium | Medium | Not yet covered. | Needs fixture proof before treating as income, corporate-action evidence, or review-only evidence; conversion evidence flags possible zero-quantity and principal-representation issues. |
