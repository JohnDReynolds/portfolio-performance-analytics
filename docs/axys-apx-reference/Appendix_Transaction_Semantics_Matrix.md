...
| `ai` | Negative interest, accrued interest, margin interest. | `fee_expense` or `income` | Usually negative for margin/negative interest; context-dependent. | `performance` | Security type, margin/accrual symbols, amount sign, local mapping or REP/report semantics. | Low to Medium | Medium | Not yet covered in packaged performance demo. | Packaged demo uses `holdings.accrued` plus `in` interest; do not infer `ai` from code alone. |
| `sa` | Sell accrued interest. | `income` or `fee_expense` pending local mapping. | Context-dependent. | `performance` or `unknown` | Bond/accrual context, cash offset, amount sign, local mapping or REP/report semantics. | Low | Medium | Not yet covered. | Requires sell-accrued-interest fixture before packaged-demo use. |
| `pa` | Purchase accrued interest or accrued-interest-related buy-side case. | `fee_expense` or `income` pending local mapping. | Context-dependent. | `performance` or `unknown` | Bond/accrual context, cash offset, amount sign, local mapping or REP/report semantics. | Low | Medium | Not yet covered. | Requires purchase-accrued-interest fixture before packaged-demo use. |
...
| `pd` | Principal paydown / bond return-of-capital case. | `income` or `corporate_action` pending local policy. | Usually positive cash with reduced principal exposure. | `performance` or `review_only` depending on configured methodology. | Bond/security type, principal/paydown context, cash movement, amount and quantity/principal signs. | Low to Medium | Medium | Not yet covered. | Needs fixture proof before treating as income, corporate-action evidence, or review-only evidence. |
