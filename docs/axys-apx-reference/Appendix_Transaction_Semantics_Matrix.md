...
| `dv` + `by` reinvestment pair | Reinvested dividend represented as income plus buy. | Income leg is `performance`; buy leg is `performance`; avoid double-counting as external flow. | Pairing evidence such as dividend-wash symbol, dates, offsetting amounts, security, and local mapping. | Not yet covered as paired reinvestment fixture. | Requires explicit double-count guards before fixture or packaged-demo inclusion. |
