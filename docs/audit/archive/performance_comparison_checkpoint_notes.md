...
- `supporting_files/source_detail.csv`: full finding-level detail.
...
- Aligned the source-detail artifact with the common left-side review columns.
...
retired. The current smoke path generates demo output from a setup workspace:
...
- Keep deep diagnostic fields such as `Code` and `Review Rank` in
  `supporting_files/source_detail.csv`, not in the main action sheets.
- Use the same left-side sort fields for the `Performance Difference Causes` sheet,
  and `source_detail.csv`: `Portfolio`, `From Date`,
