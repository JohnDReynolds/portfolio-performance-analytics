...
This file collects implementation-oriented research for a future technical reference chapter on corporate actions in Axys/APX.
...
|---|---|---|---:|---|
| Corporate actions are a distinct operational domain for Advent users; SS&C offers Advent Corporate Actions (ACA) as a separate corporate actions solution. | ACA is marketed as integrated with Axys | ACA is marketed specifically for APX | Verified | SS&C product briefs describe ACA workflows for both Axys/APX. |
...
| Return of capital | Income/cost basis transaction; `rc` is confirmed in public Axys/APX translation mapping evidence, and bond-security return of capital maps to `pd`. | Transaction-code evidence supported; native posting mechanics Unknown | Transaction-code evidence supported; native posting mechanics Unknown | Medium-High for integration mapping; Unknown native mechanics | Requires native transaction/report samples to verify cost-basis and performance treatment. The 2026-07-07 `pd` Modified Dietz research recommends treating bond-security `pd` as principal return, not client external flow, when fixed-income and cash-destination context exists. |
...
| Principal paydown | Bond/MBS principal return; cash increases and outstanding principal decreases. | ByAllAccounts mapping and Morningstar conversion evidence support `pd` as the likely transaction-code context. | Mapping evidence supports APX-adjacent `pd`, but native mechanics remain Unknown. | Medium-High for integration/conversion evidence; Unknown native mechanics | Do not infer from code alone; preserve security type, cash destination, principal/factor, and performance-report evidence. |
...
|---|---|---:|
| Which IMEX object exports transactions in Axys/APX? | Needed to audit dividend/reorg postings. | Unknown |
...
| Are there separate Axys/APX report names for corporate action review? | Unknown |
...
3. ACA is verified as an Advent corporate-actions workflow for both Axys/APX at the vendor workflow/product-claim level.
...
| CA-U017 | Exact fields in the ACA Automation Results email for Axys/APX | Axys/APX | Medium |
...
`Data Issues` worksheet than to default `Performance Difference Causes` unless
