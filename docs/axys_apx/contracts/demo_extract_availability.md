...
| transactions | `ORIGINAL_COST_DATE` | Original acquisition or cost date supplied for a configured deliver-in transaction. | Low / Medium | High | Chapter_05_Transactions.md Section 2.8 records Original_Cost_Date in the bounded public missing-cost report workflow. | Confirm the local field name, date basis, deliver-in population, and whether blank values trigger report fallback behavior. | Observed in a cited Axys missing-cost report workflow; ordinary posted-transaction extract availability is site-specific. |
| transactions | `ORIGINAL_COST` | Original cost amount supplied for a configured deliver-in transaction. | Low / Medium | High | Chapter_05_Transactions.md Section 2.8 records Original_Cost in the bounded public missing-cost report workflow. | Confirm the local field name, currency basis, deliver-in population, and whether blank values trigger report fallback behavior. | Observed in a cited Axys missing-cost report workflow; currency basis and ordinary posted-export availability require local confirmation. |
...
| transactions | `ORIGINAL_COST_DATE` | ORIGINAL_COST_DATE, ORIG_COST_DATE, COST_DATE | Original Cost Date | Report Label Inferred | The report label is documented for the cited workflow; native IMEX naming and broader version coverage remain unproven. |
| transactions | `ORIGINAL_COST` | ORIGINAL_COST, ORIG_COST | Original Cost | Report Label Inferred | The report label is documented for the cited workflow; native IMEX naming and broader version coverage remain unproven. |
...
| transactions | `ORIGINAL_COST_DATE` | REP preferred | Local discovery required | Required only for the explicitly enabled deliver-in completeness check; absence must fail setup rather than classify every row as incomplete. |
| transactions | `ORIGINAL_COST` | REP preferred | Local discovery required | Required only for the explicitly enabled deliver-in completeness check; PPAR checks presence without calculating cost basis. |
