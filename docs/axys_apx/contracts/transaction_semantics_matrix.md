...
| `ti` | Deliver in in the cited Axys missing-cost report workflow; exact site semantics remain context-dependent. | `external_flow`, `transfer` | `positive`, `none` | `external`, `neutral` | `source_workflow`, `source_destination_type`, `source_destination_symbol`, `security_type`, `quantity`, `amount`, `original_cost`, `original_cost_date`, `local_mapping` | Low Medium | Medium High For Cited Workflow | Partially documented or tested, but not fully covered. | `packaged_demo` | Packaged demo covers one site-scoped external-security deliver-in using explicit external-party and security context. Its blank original-cost amount and date drive the separately opt-in completeness finding without changing performance treatment. Code-only ti remains unknown; internal-transfer treatment requires separate local evidence. |
