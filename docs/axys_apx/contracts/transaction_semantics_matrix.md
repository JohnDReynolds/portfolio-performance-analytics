...
| `ss` | Short sale; ByAllAccounts APX mapping evidence maps `SELL / SHORT` to `ss`. | `sell` or `unknown pending review` | Positive, collateral-like, or short-proceeds-specific depending on site short-account treatment. | `performance` or `unknown` | Lowercase code, security, quantity, amount, short/security type or resulting negative exposure, cash/margin/short symbols, and source/destination context. | Medium-High for code meaning; Low to Medium for mechanics | Medium | Partially covered by packaged demo and a test-only site variant. | Packaged demo covers `ss` only in a disclosed synthetic TSLA short lifecycle using real prices and explicit source/destination context; `site_variants/short_side_trades` proves explicit short-security YAML classification as a performance sell. Code-only production treatment remains unknown. Uppercase `SS` may be reversal/delete evidence. |
| `cs` | Cover short; ByAllAccounts APX mapping evidence maps `BUY / COVER SHORT` to `cs`. | `buy` or `unknown pending review` | Negative, collateral-like, or short-cash-release-specific depending on site short-account treatment. | `performance` or `unknown` | Lowercase code, security, quantity, amount, prior or resulting short exposure, cash/margin/short symbols, and source/destination context. | Medium-High for code meaning; Low to Medium for mechanics | Medium | Partially covered by packaged demo and a test-only site variant. | Packaged demo covers `cs` only in a disclosed synthetic TSLA cover-short lifecycle using real prices and explicit source/destination context; `site_variants/short_side_trades` proves explicit short-security YAML classification as a performance buy. Code-only treatment remains unknown, and uppercase `CS` cancellation evidence stays separate. |
