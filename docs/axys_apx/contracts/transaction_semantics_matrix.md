...
| `epus` | Management fee or fee-related token in conversion evidence. | `fee_expense` | `negative` | `performance` | `token_role`, `fee_symbol`, `local_mapping` | Low Medium | Medium | Documented backlog; no dedicated fixture yet. |  | Backlog pending evidence that epus appears as a standalone transaction code rather than a fee token, label, or special-security context. |
