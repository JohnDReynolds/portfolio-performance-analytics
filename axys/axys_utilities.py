...
def derive_secperf_for_all_periods(
    ...
    """Fast version using pre-partitioning of secperf by period.
    ...
    required_portperf = ("FROM_DATE", "THRU_DATE", "PORT_RETURN")
    required_secperf = ("FROM_DATE", "THRU_DATE")

    missing_pp = [c for c in required_portperf if c not in portperf.columns]
    if missing_pp:
        raise ValueError(f"portperf missing columns: {missing_pp}")

    missing_sp = [c for c in required_secperf if c not in secperf.columns]
    if missing_sp:
        raise ValueError(f"secperf missing columns: {missing_sp}")

    # --- Validate unique portperf periods ---
    dup_periods = portperf.group_by(["FROM_DATE", "THRU_DATE"]).len().filter(pl.col("len") > 1)
    if dup_periods.height > 0:
        raise ValueError(f"Duplicate portperf periods: {dup_periods.head(10).to_dicts()}")

    # --- Pre-group secperf ONCE (big win) ---
    secperf_groups = secperf.partition_by(
        ["FROM_DATE", "THRU_DATE"],
        as_dict=True,
    )
    ...
    for row in portperf.iter_rows(named=True):
        ...
        if secperf_period is None or secperf_period.height == 0:
            raise ValueError(f"No secperf rows for period {key}")

        derived = _derive_reconciled_weights(secperf_period, port_return)

        # If the derived weights do not give sumof(wgt * ret) =~ portperf.ret, then fail.
        if not _is_reconciled(derived, tolerance=0.00001):
            return None

        derived_frames.append(derived)

    if not derived_frames:
        return secperf.head(0)

    secperf_derived = pl.concat(derived_frames, how="vertical", rechunk=True)

    # --- Final defensive check ---
    if secperf_derived.height != secperf.height:
        raise ValueError(
            "Row count mismatch: " f"derived={secperf_derived.height}, input={secperf.height}"
        )

    return secperf_derived


def load_and_validate_portperf_and_secperf(
    portperf_path: str,
    secperf_path: str,
    ...
    """Load and validate filtered portperf and secperf CSV data.
    ...
    Args:
        ...
        from_date: Optional lower date bound. Keeps rows where from_date <= FROM_DATE.
        thru_date: Optional upper date bound. Keeps rows where THRU_DATE <= thru_date.

    Returns:
        A tuple of:
            - filtered/validated portperf DataFrame
            - filtered/validated secperf DataFrame

    Raises:
        ValueError: If any validation fails.
    """
    portperf_df: pl.DataFrame = _scan_csv_selected_columns(
        portperf_path,
    ...
    secperf_df: pl.DataFrame = _scan_csv_selected_columns(
        secperf_path,
    ...
    _validate_secperf_uniqueness(secperf_df)
    _validate_portperf_row_date_order(portperf_df)
    _validate_portperf_continuous_periods(portperf_df)
    _validate_secperf_periods_match_portperf(portperf_df, secperf_df)

    return portperf_df, secperf_df
...
def _derive_reconciled_weights(
    ...
    *,
    ...
    This function tries to return a usable result even when the input has weak implied weights,
    null-like numeric values, or a target portfolio return that is awkward for the preferred
    closed-form tilt.
    ...
    If the closed-form tilt is unstable or produces materially negative weights, the function
    falls back to:
    ...
    Raises:
        ValueError: Required columns are missing, the DataFrame is empty, or parameter values
            are invalid.
    """
    ...
    returns = [_finite_or_default(value, default=0.0) for value in returns]
    contributions = [_finite_or_default(value, default=0.0) for value in contributions]
    begin_weights = [_finite_or_default(value, default=float("nan")) for value in begin_weights]
    begin_market_values = [
        _finite_or_default(value, default=float("nan")) for value in begin_market_values
    ]
    ...
    adjusted_weights, recon_method, recon_notes = _solve_adjusted_weights(
...
def _is_reconciled(
    ...
    *,
    ...
    if result_df.height == 0:
        raise ValueError("result_df must contain at least one row.")

    required_columns = (weight_col, return_col, target_col)
...
def _scan_csv_selected_columns(
    ...
    *,
    ...
    """Read a CSV lazily, project only requested columns, and apply optional filters.

    Args:
        path: CSV file path.
        requested_columns: Exact column names to read.
        portfolio_code: Optional portfolio filter. When provided, keep only rows where
            PORTFOLIO_CODE == portfolio_code.
        from_date: Optional lower date bound. When provided, keep only rows where
            from_date <= FROM_DATE.
        thru_date: Optional upper date bound. When provided, keep only rows where
            THRU_DATE <= thru_date.
    ...
    if missing_columns:
        raise ValueError(
            f"Missing columns in {path!r}: {missing_columns}. "
    ...
    if portfolio_code is not None:
        lazy_frame = lazy_frame.filter(pl.col("PORTFOLIO_CODE") == portfolio_code)

    if from_date is not None:
        lazy_frame = lazy_frame.filter(pl.lit(from_date) <= pl.col("FROM_DATE"))

    if thru_date is not None:
        lazy_frame = lazy_frame.filter(pl.col("THRU_DATE") <= pl.lit(thru_date))
...
def _solve_adjusted_weights(
    ...
    if closed_form_weights is not None:
        closed_form_return = _weighted_return(closed_form_weights, returns)
    ...
    bisection_weights = _solve_bisection_tilt(
        anchor_weights=anchor_weights,
        returns=returns,
        target_return=target_return,
        match_tolerance=match_tolerance,
    ...
    if bisection_weights is not None:
        bisection_return = _weighted_return(bisection_weights, returns)
    ...
    two_security_weights = _solve_two_security_fallback(
        security_ids=security_ids,
        anchor_weights=anchor_weights,
        returns=returns,
        target_return=target_return,
...
def _solve_bisection_tilt(
    ...
    for left_point, right_point in zip(valid_points[:-1], valid_points[1:]):
        ...
        for _ in range(max_iterations):
            ...
            mid_weights = _weights_from_lambda(
                anchor_weights=anchor_weights,
                returns=returns,
            ...
            mid_residual = _weighted_return(mid_weights, returns) - target_return
...
def _solve_two_security_fallback(
    ...
    the feasible range of security returns, but it can move far away from the anchor weights.
    To keep it less arbitrary, it chooses the bracketing pair with the largest combined anchor
    weight.
...
def _validate_parameters(
    ...
    if not math.isfinite(return_epsilon) or return_epsilon <= 0.0:
        raise ValueError("return_epsilon must be a positive finite float.")
    if not math.isfinite(match_tolerance) or match_tolerance <= 0.0:
        raise ValueError("match_tolerance must be a positive finite float.")
    if not math.isfinite(near_zero_weight) or near_zero_weight <= 0.0:
        raise ValueError("near_zero_weight must be a positive finite float.")


def _validate_portperf_continuous_periods(portperf_df: pl.DataFrame) -> None:
    ...
    if duplicate_periods.height > 0:
        ...
        raise ValueError(
            "portperf period validation failed. Duplicate periods found. "
            f"Sample duplicates: {sample_rows}"
        )
    ...
    if continuity_check_df.height > 0:
        ...
        raise ValueError(
            "portperf continuity validation failed. "
            "The periods are not continuous; at least one gap or overlap exists. "
            f"Sample discontinuities: {sample_rows}"
        )
...
def _validate_portperf_row_date_order(portperf_df: pl.DataFrame) -> None:
    ...
    if invalid_rows.height > 0:
        ...
        raise ValueError(
            "portperf date-order validation failed. "
            "Found rows where FROM_DATE > THRU_DATE. "
            f"Sample invalid rows: {sample_rows}"
        )


def _validate_required_columns(secperf_df: pl.DataFrame) -> None:
    """Validate that the required secperf columns are present."""
    missing_columns = [column for column in _REQUIRED_COLUMNS if column not in secperf_df.columns]
    if missing_columns:
        raise ValueError(f"Missing required columns: {missing_columns}")
